...
def execute_tools(state: List[BaseMessage]) -> List[BaseMessage]:
    last_ai_message: AIMessage = state[-1] # Extract tool calls from the AI message
    if not hasattr(last_ai_message, "tool_calls") or not last_ai_message.tool_calls:
        return []

    tool_messages = [] # Process the AnswerQuestion or ReviseAnswer tool calls to extract search queries
    ...
    for tool_call in last_ai_message.tool_calls:
        if tool_call["name"] in ["AnswerQuestion", "ReviseAnswer"]:
            ...
            query_results = {} # Execute each search query using the tavily tool
        ...
        tool_calls=[
            {
                ...
                "id": "call_7h6ryZlLSbUNkMPtKlyq47Wu",
            }
        ],
    )
]
